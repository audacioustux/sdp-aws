...
const nm = (name: String) =>
  `${pulumi.getProject()}-${pulumi.getStack()}-${name}`;
...
const k8sServiceHosts = k8s.core.v1.Endpoints.get(
  "kubernetes-endpoint",
  "kubernetes",
  { provider }
).subsets.apply((subsets) =>
...
const scheduledChangeRule = new aws.cloudwatch.EventRule(
  scheduledChangeRuleName,
  {
...
const spotInterruptionRule = new aws.cloudwatch.EventRule(
  spotInterruptionRuleName,
  {
    eventPattern: JSON.stringify({
      source: ["aws.ec2"],
...
const instanceStateChangeRule = new aws.cloudwatch.EventRule(
  instanceStateChangeRuleName,
  {
    eventPattern: JSON.stringify({
      source: ["aws.ec2"],
  ...
  aws.iam.getPolicyDocumentOutput({
    statements: [
      ...
      },
      {
        ...
        effect: "Allow",
        resources: [
          `arn:${partitionId}:ssm:${regionId}::parameter/aws/service/*`,
        ],
      ...
      },
      {
        ...
        effect: "Allow",
        resources: [
          `arn:${partitionId}:sqs:${regionId}:${accountId}:${ec2InterruptionQueueName}`,
        ],
      ...
      },
      {
        ...
        effect: "Allow",
        resources: [
          `arn:${partitionId}:iam::${accountId}:role/${eksNodeRoleName}`,
        ],
      ...
      },
      {
        ...
        effect: "Allow",
        resources: [
          `arn:${partitionId}:eks:${regionId}:${accountId}:cluster/${eksClusterName}`,
        ],
...
const accountId = await aws
  .getCallerIdentity()
  .then((identity) => identity.accountId);

const karpenterControllerPolicyName = nm("karpenter-controller-policy");
const KarpenterControllerPolicy = new aws.iam.Policy(
  karpenterControllerPolicyName,
  {
    policy: pulumi
      .all([eksCluster.name, EC2InterruptionQueue.name, eksNodeRole.name])
      .apply(
